...
handler = logging.FileHandler("./log/631_Vggish_Regression_Arousal_Latest_log.txt")
...
def svr(i):
    ...
    # Save
    df = pd.DataFrame(data={"Vggish_prediction_A": prediction_test, "Vggish_groundtruth_A": test_y.values.tolist()})
    df.to_csv(f"./Prediction_202106_Ratio631/CV{i}_Vggish_Arousal_0621.csv")
    print("save success!")
...
if __name__ == '__main__':
    ...
    print(f"Vggish Regression Average Results of Arousal: RMSE.= {mean(average_rmse_test)}, Spearman = {mean(average_pearson_test)}, CCC = {mean(average_ccc_test)}")
    logger.info(
        f"/n/n/n Vggish Regression Average Results of Arousal: RMSE.= {mean(average_rmse_test)}, Spearman = {mean(average_pearson_test)}, CCC = {mean(average_ccc_test)}")
    pool.join()