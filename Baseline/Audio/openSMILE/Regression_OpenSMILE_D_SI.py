...
def svr(i):
    ...
    train_y = pd.read_csv(f'./CV_Features_631/RegressionFeatures/Train_CV_{i}.csv').iloc[:, 7]
    validation_x = pd.read_csv(f'./CV_Features_631/RegressionFeatures/Validation_CV_{i}.csv').iloc[:, 9:]
    validation_y = pd.read_csv(f'./CV_Features_631/RegressionFeatures/Validation_CV_{i}.csv').iloc[:, 7]
    test_x = pd.read_csv(f'./CV_Features_631/RegressionFeatures/Test_CV_{i}.csv').iloc[:, 9:]
    test_y = pd.read_csv(f'./CV_Features_631/RegressionFeatures/Test_CV_{i}.csv').iloc[:, 7]
    ...
    # Save
    df = pd.DataFrame(data={"opensmile_prediction_D": prediction_test, "opensmile_groundtruth_D": test_y.values.tolist()})
    df.to_csv(f"./Prediction_202106_Ratio631/CV{i}_opensmile_Dominance_0621.csv")
    print("save success!")
...
if __name__ == '__main__':
    ...
    print(f"OpenSMILE Regression Average Results of Dominance: RMSE.= {mean(average_rmse_test)}, Spearman = {mean(average_pearson_test)}, CCC = {mean(average_ccc_test)}")
    logger.info(
        f"/n/n/n OpenSMILE Regression Average Results of Dominance: RMSE.= {mean(average_rmse_test)}, Spearman = {mean(average_pearson_test)}, CCC = {mean(average_ccc_test)}")
    pool.join()