...
handler = logging.FileHandler("./log/631_OpenSMILE_Classificaiton_Latest_log.txt")
...
def svm(i):
    train_x = pd.read_csv(f'./CV_Features_631/ClassificationFeatures/Train_CV_{i}.csv').iloc[:, 8:]
    train_y = pd.read_csv(f'./CV_Features_631/ClassificationFeatures/Train_CV_{i}.csv').iloc[:, 3]
    validation_x = pd.read_csv(f'./CV_Features_631/ClassificationFeatures/Validation_CV_{i}.csv').iloc[:, 8:]
    validation_y = pd.read_csv(f'./CV_Features_631/ClassificationFeatures/Validation_CV_{i}.csv').iloc[:, 3]
    test_x = pd.read_csv(f'./CV_Features_631/ClassificationFeatures/Test_CV_{i}.csv').iloc[:, 8:]
    test_y = pd.read_csv(f'./CV_Features_631/ClassificationFeatures/Test_CV_{i}.csv').iloc[:, 3]
    ...
    for idx, gamma in enumerate(tqdm(gamma_range)):
        ...
        # ------------
        time0 = time()
        logger.info(f"Ratio_631 >>>>>>>Training CV = {i}/10, Start Trainng {idx + 1}/{len(gamma_range)}>>>>>>>")
        print(f"Ratio_631 >>>>>>>Training CV = {i}/10, Start Training {idx + 1}/{len(gamma_range)}>>>>>>>")
        ...
        # Logger
        logger.info(f"Ratio_631 Validation Gamma >>> Acc. = {round(accuracy_valid,4)}, F1-Score = {round(f1_valid,4)}, AUC = {round(auc_valid,4)}")
        print(f"Ratio_631 Validation Gamma >>> Acc. = {round(accuracy_valid,4)}, F1-Score = {round(f1_valid,4)}, AUC = {round(auc_valid,4)}")
    ...
    for idx, c in enumerate(tqdm(C)):
        time0 = time()
        logger.info(f"Ratio_631 >>>>>>>CV = {i}/10, Fine-Tuining C, Start Trainng {idx + 1}/{len(C)}>>>>>>>")
        print(f"Ratio_631 >>>>>>> CV = {i}/10, Fine-Tuining C, Start Training {idx + 1}/{len(C)}>>>>>>>")
        clf = OneVsRestClassifier(
            SVC(kernel='rbf',  #
                gamma=best_gamma,
                C=c,  # default
        ...
        auc_valid = roc_auc_score(y_binary_valid, result_valid, average='micro')
        # Logger
        logger.info(f"Ratio_631 Validation C >>> Acc. = {accuracy_valid}, F1-Score = {f1_valid}, AUC = {auc_valid}")
        print(f"Ratio_631 Validation C >>> Acc. = {accuracy_valid}, F1-Score = {f1_valid}, AUC = {auc_valid}")
        print(datetime.datetime.fromtimestamp(time() - time0).strftime("%M:%S:%f"))
    ...
    print(
        f"Validation >>> Best gamma = {best_gamma}, Best C = {best_c}, Acc. ={round(best_acc,4)}, F1-Score = {round(best_f1,4)}, AUC = {round(best_auc,4)}\n")
    logger.info(
        f"Validation >>> Best gamma = {best_gamma}, Best C = {best_c}, Acc. ={round(best_acc,4)}, F1-Score = {round(best_f1,4)}, AUC = {round(best_auc,4)}\n")
    ...
    logger.info(f"CV = {i}, Test >>> gamma = {best_gamma}, Acc. ={test_accuracy}, F1-Score = {test_f1}, AUC = {test_auc}")

    # save
    result_test = clf_best_test.predict_proba(x_test_std)
    df = pd.DataFrame(result_test)
    df.to_csv(f"./Prediction_202106_Ratio631/categorical_opensmile_6pnn_20210621_prediction_CV{i}_Gamma_{round(best_gamma,4)}_C_{best_c}_ACC_{test_accuracy}_F1_{test_f1}_AUC_{test_auc}.csv")
    df2 = pd.DataFrame(y_test)
    df2.to_csv(f"./Prediction_202106_Ratio631/categorical_opensmile_6pnn_20210324_GT_CV{i}.csv")
...
if __name__ == '__main__':
    ...
    print(f"OpenSMILE Classification Average Results: Acc.= {mean(average_acc_test)}, F1 = {mean(average_f1_test)}, AUC = {mean(average_auc_test)}")
    print(f"average_acc_test = {average_acc_test},/n average_f1_test={average_f1_test},/n average_auc_test = {average_auc_test}")
    logger.info(
        f"/n/n/n OpenSMILE Classification Average Results: Acc.= {mean(average_acc_test)}, F1 = {mean(average_f1_test)}, AUC = {mean(average_auc_test)}")
    pool.join()
