...
def svr(i):
    ...
    # Save
    df = pd.DataFrame(data={"opensmile_prediction_p": prediction_test, "opensmile_groundtruth_p": test_y.values.tolist()})
    df.to_csv(f"./Prediction_202106_Ratio631/CV{i}_opensmile_pleasure_0621.csv")
    print("save success!")
...
if __name__ == '__main__':
    ...
    print(f"OpenSMILE Regression Average Results of Pleasure: RMSE.= {mean(average_rmse_test)}, Spearman = {mean(average_pearson_test)}, CCC = {mean(average_ccc_test)}")
    logger.info(
        f"/n/n/n OpenSMILE Regression Average Results of Pleasure: RMSE.= {mean(average_rmse_test)}, Spearman = {mean(average_pearson_test)}, CCC = {mean(average_ccc_test)}")
    pool.join()