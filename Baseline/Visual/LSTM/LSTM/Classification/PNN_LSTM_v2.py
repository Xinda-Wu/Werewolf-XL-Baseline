...
handler = logging.FileHandler("./log/20210628_LSTM_Classification_v2_Latest_log.txt")
...
writer_train = SummaryWriter(log_dir='./runs/Classification_v2/PNN_train/')
writer_valid = SummaryWriter(log_dir='./runs/Classification_v2/PNN_valid/')
writer_test = SummaryWriter(log_dir='./runs/Classification_v2/PNN_test/')
...
# Early stopping
parser.add_argument('-p', '--patience', default=3, type=int,
...
def train(train_loader, featureExtractor, model, criterion, optimizer, epoch, device, cv):
    ...
    writer_train.add_scalar(f"CV_{cv}_Accuracy", train_accuracy, epoch)
    return loss_average
...
def valid(valid_loader, featureExtractor, model, criterion, optimizer, epoch, device, cv):
    ...
    with torch.no_grad():
        ...
        writer_valid.add_scalar(f"CV_{cv}_Accuracy", valid_accuracy, epoch)
...
def test(test_loader, featureExtractor, model, epoch, device, cv):
    ...
    with torch.no_grad():
        ...
        df.to_csv(f"./Prediction/20210628/CV_{cv}_Epoch_{epoch}_ACC_{test_accuracy}_eval_pnn.csv")

        pro = np.array(prediction_prob)
        df2 = pd.DataFrame(pro)
        df2.to_csv(f"./Prediction/20210628/CV_{cv}_Epoch_{epoch}_ACC_{test_accuracy}_Categorical_lstm_6pnn_202106.csv")
