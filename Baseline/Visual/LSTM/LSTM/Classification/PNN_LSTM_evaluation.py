...
def test(test_loader, featureExtractor, model, epoch, device, cv):
    ...
    with torch.no_grad():
        ...
        df.to_csv(f"./Prediction/20210629_2/CV_{cv}_Epoch_{epoch}_ACC_{test_accuracy}_eval_pnn_2.csv")

        pro = np.array(prediction_prob)
        df2 = pd.DataFrame(pro)
        df2.to_csv(f"./Prediction/20210629_2/CV_{cv}_Epoch_{epoch}_ACC_{round(test_accuracy,5)}_F1_{round(f1,5)}_AUC_{round(auc,5)}_Categorical_lstm_6pnn_202106_2.csv")
        print(f"save cv {cv}")
...
if __name__ == '__main__':
    ...
    for i in range(1, 11):
        ...
        average_f1_test.append(test_result[1])
        average_auc_test.append(test_result[2])
    print(f"LSTM Classificaiton Average Results of Arousal: ACC.= {np.average(average_acc_test)},\
                F1 Score = {np.average(average_f1_test)}, AUC = {np.average(average_auc_test)}")
