...
# Early stopping
parser.add_argument('-p', '--patience', default=6, type=int,
...
def test(test_loader, featureExtractor, model, epoch, device, cv):
    ...
    with torch.no_grad():
        ...
        df.to_csv(f"./Prediction_202106/Weighted_CV_{cv}_Epoch_{epoch}_ACC_{test_accuracy}_eval_pnn.csv")

        pro = np.array(prediction_prob)
        df2 = pd.DataFrame(pro)
        df2.to_csv(f"./Prediction_202106/Weighted_CV_{cv}_Epoch_{epoch}_ACC_{test_accuracy}_Categorical_lstm_6pnn_202106.csv")
...
def main():
    ...
    for i in range(1,11): #
        ...
        early_stopping = EarlyStopping(patience=args.patience, verbose=True,
                                       path=f"./checkpoints/weighted_early_stopping_checkpoint_CV_{i}.pt")
        ...
        for epoch in range(args.start_epochs,  args.epochs):
            ...
            if epoch % 5 == 0:
                ...
                avg_valid_loss.append(loss)
                # save model
                torch.save(model, f'./checkpoints_202106/Weighted_CV_{i}_Classification_lstm_loss_' + str("%.4f" % loss) + '.pth')
                ...
                if early_stopping.early_stop:
                    print("Early stopping")
                    torch.save(model, f'./checkpoints_202106/Weighted_CV_{i}_Classification_lstm_loss_{str("%.4f" % loss)}_earlyStopping_{epoch}.pth')
                    break

                    # -------------------
                    # Test the model
                    # -------------------
        model.load_state_dict(torch.load(f"./checkpoints_202106/weighted_early_stopping_checkpoint_CV_{i}.pt"))
