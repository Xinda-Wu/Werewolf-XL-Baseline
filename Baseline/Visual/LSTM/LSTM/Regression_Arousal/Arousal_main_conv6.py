...
from Arousal_TestFileDataset import *
from Arousal_ValidFileDataset import *
from Arousal_TrainFileDataset import *
...
handler = logging.FileHandler("./log/20210628_LSTM_Regression_Arousal_Latest_log.txt")
...
writer_train = SummaryWriter(log_dir='./runs/Regression_2/Arousal/train/')
writer_valid = SummaryWriter(log_dir='./runs/Regression_2/Arousal/valid/')
writer_test = SummaryWriter(log_dir='./runs/Regression_2/Arousal/test/')
...
def valid(valid_loader, featureExtractor, model, criterion, optimizer, epoch, device, cv):
    ...
    with torch.no_grad():
        for i, videoFrames in enumerate(tqdm(valid_loader)):
            ...
            # print(f'>>>>> videoFrames = {videoFrames}')
...
def test(test_loader, featureExtractor, model, device, cv):
    ...
    with torch.no_grad():
        ...
        df.to_csv(f"./Prediction/20210628/eval_arousal_CV_{cv}_RMSE_{rmse}_Spearman_{spearman}_CCC_{ccc}.csv")
