...
handler = logging.FileHandler("./log/Hog_Regression_Dominance_Latest_log.txt")
...
def svr(i):
    train_x = pd.read_csv(f'./CV_Features/RegressionFeatures/Train_CV_{i}.csv').iloc[:, 9:]
    train_y = pd.read_csv(f'./CV_Features/RegressionFeatures/Train_CV_{i}.csv').iloc[:, 7]
    validation_x = pd.read_csv(f'./CV_Features/RegressionFeatures/Validation_CV_{i}.csv').iloc[:, 9:]
    validation_y = pd.read_csv(f'./CV_Features/RegressionFeatures/Validation_CV_{i}.csv').iloc[:, 7]
    test_x = pd.read_csv(f'./CV_Features/RegressionFeatures/Test_CV_{i}.csv').iloc[:, 9:]
    test_y = pd.read_csv(f'./CV_Features/RegressionFeatures/Test_CV_{i}.csv').iloc[:, 7]
    ...
    # Save
    df = pd.DataFrame(data={"hog_prediction_D": prediction_test, "hog_groundtruth_D": test_y.values.tolist()})
    df.to_csv(f"./Prediction_202106/CV{i}_hog_Dominance_0621.csv")
    print("save success!")
...
if __name__ == '__main__':
    ...
    print(f"Hog Regression Average Results of Dominance: RMSE.= {mean(average_rmse_test)}, Spearman = {mean(average_pearson_test)}, CCC = {mean(average_ccc_test)}")
    logger.info(
        f"/n/n/n Hog Regression Average Results of Dominance: RMSE.= {mean(average_rmse_test)}, Spearman = {mean(average_pearson_test)}, CCC = {mean(average_ccc_test)}")
    pool.join()