...
handler = logging.FileHandler("./log/hog_Classificaiton_Latest_311_log.txt")
...
def svm(i):
    ...
    validation_y = pd.read_csv(f'./CV_Features_631/ClassificationFeatures/Validation_CV_{i}.csv').iloc[:, 4]
    ...
    df.to_csv(f"./Prediction_ratio631/Ratio631_C{i}_categorical_hog_6pnn_20210621_prediction_Gamma_{round(best_gamma,4)}_ACC_{test_accuracy}_F1_{test_f1}_AUC_{test_auc}.csv")
    df2 = pd.DataFrame(y_test)
    df2.to_csv(f"./Prediction_ratio631/Ratio631_C{i}_categorical_hog_6pnn_20210324_GT.csv")
...
if __name__ == '__main__':
    ...
    futures = [pool.apply_async(func=svm, args=[i]) for i in range(1, 10)]
    ...
    print(f"Ratio631 HOG Classification Average Results: Acc.= {mean(average_acc_test)}, F1 = {mean(average_f1_test)}, AUC = {mean(average_auc_test)}")
    print(
        f"Ratio631 average_acc_test = {average_acc_test},/n average_f1_test={average_f1_test},/n average_auc_test = {average_auc_test}")
    logger.info(
        f"Ratio631 HOG Classification Average Results: Acc.= {mean(average_acc_test)}, F1 = {mean(average_f1_test)}, AUC = {mean(average_auc_test)}")
    pool.join()
