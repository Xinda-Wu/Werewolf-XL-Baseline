...
1。 lossMIN调整
2。 tensorboard一起优化
sudo python3  -m torch.distributed.launch Evaluation_lstm_Dominance.py
...
def main():
    ...
    # Load Data
    test_dataset = Dominance_FileDatabase.FlameSet()
    test_loader = torch.utils.data.DataLoader(test_dataset,
                                              batch_size=args.test_batch_size, shuffle=False,
                                              num_workers=args.workers, pin_memory=False)

    # Multiple GPU training , load device
    os.environ["CUDA_VISIBLE_DEVICES"] = '9'  # 想使用的GPU编号
    ...
    #
    # torch.distributed.init_process_group(backend="nccl", init_method='tcp://localhost:23490', rank=0, world_size=1)
    # model_CKPT = torch.load('/home/xinda/werewolf_lstm/werewolf_video/evaluation_model/lstm_loss_nan.pth.tar')
    # model.to(device)
    ...
    model_CKPT = torch.load('/home/xinda/werewolf_lstm/werewolf_video/Dominance_saved_model/lstm_loss_80.2279.pth.tar')
    model_state = model_CKPT['state_dict']
    ...
    with torch.no_grad():
        ...
        df.to_csv("eval_dominance.csv")
        print("eval_dominance.csv save success!!")
