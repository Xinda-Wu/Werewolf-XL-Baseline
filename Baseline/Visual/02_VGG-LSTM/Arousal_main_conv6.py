...
import Arousal_TrainFileDataset
...
writer_train = SummaryWriter(log_dir='./runs/Arousal_train/')
writer_test = SummaryWriter(log_dir='./runs/Arousal_test')
...
def main():
    ...
    # Load Data
    train_dataset = Arousal_TrainFileDataset.FlameSet()  # for training 0.7
    test_dataset = Arousal_FileDatabase.FlameSet()  # for test  0.3
    ...
    for epoch in range(args.start_epochs, args.epochs):
        ...
        if epoch % 5 != 0:
            torch.save({'epoch': epoch,
                        ...
                        'test_best_loss': loss, },
                       './saved_model_Arousal/lstm_loss_' + str("%.4f" % loss) + '.pth.tar')
