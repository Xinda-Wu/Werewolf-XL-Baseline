...
writer_train = SummaryWriter(log_dir='./runs/train_2/')
writer_test = SummaryWriter(log_dir='./runs/test_2')
...
def test(test_loader, featureExtractor, model, criterion, optimizer, epoch, device):
    ...
    with torch.no_grad():
        ...
        writer_test.add_scalar("MSELoss", losses / 12, epoch)
        return losses
...
def main():
    ...
    # Load Data
    train_dataset = TrainFileDataset.FlameSet()  # for training 0.7
    test_dataset = FileDatabase.FlameSet()  # for test  0.3
    ...
    torch.distributed.init_process_group(backend="nccl", init_method='tcp://localhost:23459', rank=0, world_size=1)
    ...
    for epoch in range(args.start_epochs, args.epochs):
        ...
        if epoch % 5 != 0:
            torch.save({'epoch': epoch,
                        ...
                        'test_best_loss': loss, },
                       './saved_model2/lstm_loss_' + str("%.4f" % loss) + '.pth.tar')
