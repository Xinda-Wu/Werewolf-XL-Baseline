"""
吴鑫达模型优化：
sudo python3 -m torch.distributed.launch Dominance_main_conv6.py
"""
import argparse
import torch
...
import Dominance_TrainFileDataset
...
writer_train = SummaryWriter(log_dir='./runs/Dominance_train/')
writer_test = SummaryWriter(log_dir='./runs/Dominance_test')
...
def main():
    ...
    # Load Data
    train_dataset = Dominance_TrainFileDataset.FlameSet()  # for training 0.7
    test_dataset = Dominance_FileDatabase.FlameSet()  # for test  0.3
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = '0,6'  # 想使用的GPU编号
    ...
    torch.distributed.init_process_group(backend="nccl", init_method='tcp://localhost:23461', rank=0, world_size=1)
    ...
    for epoch in range(args.start_epochs, args.epochs):
        ...
        if epoch % 5 != 0:
            torch.save({'epoch': epoch,
                        ...
                        'test_best_loss': loss, },
                       './Dominance_saved_model/lstm_loss_' + str("%.4f" % loss) + '.pth.tar')
