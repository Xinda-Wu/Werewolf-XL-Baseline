...
class FlameSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(imgsPath) >=16:
            ...
            # print("frameIndexList",frameIndexList)
            inputFrameList = [imgsPath[frameIndexList[i]] for i in range(16)]
            # print("inputFrameList", inputFrameList)
        else:
            repeat = 16 -len(imgsPath)
            repeatFrame = imgsPath[-1]
            for i in range(repeat):
                imgsPath.append(repeatFrame)
            # print("少于16张，填充后：", imgsPath)
