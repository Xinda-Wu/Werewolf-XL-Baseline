...
1。 lossMIN调整
2。 tensorboard一起优化
sudo python3 -m torch.distributed.launch main_dominance.py
"""
import argparse
import torch
...
import FileDatabase_dominance
import TrainFileDataset_dominance
from VGGFace import VGG_Face
from torch.utils.tensorboard import SummaryWriter
from LSTM import FineTuneLstmModel
import random
from torch.nn.parallel import DistributedDataParallel as DDP
# Writer will output to ./runs/ directory by default
writer_train= SummaryWriter(log_dir='./runs_dominance/train/')
writer_test = SummaryWriter(log_dir='./runs_dominance/test')
...
def train(train_loader, featureExtractor, model, criterion, optimizer, epoch,device):
    ...
    writer_train.add_scalar("MSELoss_dominance",loss_all, epoch)
    ...
    with torch.no_grad():
        for i, videoFrames in enumerate(tqdm(test_loader)):
            ...
            for k in range(circle):
                ...
                output_mean = torch.mean(output).unsqueeze(0)
                #         output_mean = output_mean.unsqueeze(0)
                loss = criterion(output_mean, label)
                losses +=loss.item()
                # print("output_mean = {0}, target = {1}, loss = {2}".format(output_mean, label, loss))

       # 与train 的loss大小规模相近（可以在同一张图中显示）
        writer_test.add_scalar("MSELoss_dominance", losses/12, epoch)
...
def main():
    ...
    # Load Data
    train_dataset = TrainFileDataset_dominance.FlameSet()  # for training 0.7
    test_dataset = FileDatabase_dominance.FlameSet()  # for test  0.3
    ...
    torch.distributed.init_process_group(backend="nccl",init_method='tcp://localhost:23460', rank=0, world_size=1)
    ...
    for epoch in range(args.start_epochs, args.epochs):
        ...
        if epoch%5!= 0:
            torch.save({'epoch': epoch,
                        ...
                        'test_best_loss': loss, },
                       './saved_model_dominance/lstm_dominance_loss_' + str("%.4f" % loss) + '.pth.tar')
